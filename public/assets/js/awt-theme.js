 ...
 *
 * - Exposes `window.AwtTheme.apply(variant, mode)` for the settings page to
...
(function () {
    ...
    function apply(variant, mode) {
        ...
        // The dark-mode CSS is scoped to html.awt-theme-dark, so the class has to
        ...
        // Drop any existing awt-theme-* class and add the active one. High
        ...
        VARIANTS.forEach(v => html.classList.remove('awt-theme-' + v));
        if (renderedVariant === 'high-contrast') html.classList.add('awt-theme-dark');
        html.classList.add('awt-theme-' + renderedVariant);

        html.setAttribute('data-bs-theme', bsTheme);
        html.setAttribute('data-awt-theme', renderedVariant);
        html.setAttribute('data-awt-theme-mode', mode);
    }

    function current() {
        return {
            variant: html.getAttribute('data-awt-theme') || 'lake',
            mode: html.getAttribute('data-awt-theme-mode') || 'light',
        };
    }
    ...
    window.addEventListener('storage', (e) => {
        if (e.key !== 'awt:theme:changed' || !e.newValue) return;
        try {
