/* ============================================================================
 * awt-table.js — shared pagination engine for server-paginated tables
 * ----------------------------------------------------------------------------
 ...
 *
 * Markup contract — produced by <x-awt-table> (resources/views/components):
 *
 *   [data-awt-table]            the region; carries id, scope, route, state
 *   [data-awt-table-viewport]   the only part that is ever replaced
 *   .pagination a               intercepted (Laravel's paginator markup)
 *   [data-awt-table-link]       intercepted (a filter rendered as a link)
 *   [data-awt-table-form]       intercepted on submit AND on change
 *
...
(function () {
    ...
    if (window.__awtTableBound) return;
    window.__awtTableBound = true;

    var STORE_PREFIX = 'awt.table.v1:';
    var DT_STORE_PREFIX = 'awt.dt.v1:';
     ...
     * data-awt-table-form="admin-usage-events".
     */
    function regionOf(el, explicitId) {
        if (explicitId) {
            var byId = null;
            Array.prototype.slice.call(document.querySelectorAll('[data-awt-table]')).some(function (region) {
                if (region.dataset.awtTableId === explicitId) { byId = region; return true; }
        ...
        return el && el.closest ? el.closest('[data-awt-table]') : null;
    }

    function viewportOf(region) {
        return region ? region.querySelector('[data-awt-table-viewport]') : null;
    }
    ...
    function storageKey(region) {
        return STORE_PREFIX + (region.dataset.awtTableScope || 'anon') +
            ':' + (region.dataset.awtTableRoute || '') +
            ':' + (region.dataset.awtTableId || '');
    }

    /** The state the server says this region is currently rendering. */
    function currentState(region) {
        try {
            return JSON.parse(region.dataset.awtTableState || '{}') || {};
        } catch (e) {
            return {};
        }
    }
    ...
    function urlForState(region, state) {
        var base = region.dataset.awtTableEndpoint || window.location.pathname;
        var url;
     ...
     * region. Marking it data-awt-table-sync="<table id>" re-points it after
     * every load. `page` is dropped: an export is the whole filtered set, not
     * the page on screen.
     */
    function syncLinks(region, url) {
        var id = region.dataset.awtTableId || '';
        var params;
        ...
        Array.prototype.slice.call(document.querySelectorAll('[data-awt-table-sync="' + id + '"]')).forEach(function (link) {
            var href;
    ...
    function clearError(region) {
        var existing = region.querySelector('[data-awt-table-error]');
    ...
    function showError(region, url) {
        ...
        box.className = 'awt-table-error';
        box.setAttribute('data-awt-table-error', '');
        box.setAttribute('role', 'alert');
        box.innerHTML =
            '<span class="awt-table-error-text">That page could not be loaded. The table below is unchanged.</span>' +
            '<button type="button" class="btn btn-sm btn-outline-secondary" data-awt-table-retry>Retry</button>';
        box.querySelector('[data-awt-table-retry]').dataset.awtTableUrl = url;
    ...
    function load(region, url) {
        ...
        if (region.__awtPendingUrl === url) return;

        var viewport = viewportOf(region);
        if (!viewport) return;

        if (region.__awtAbort) {
            try { region.__awtAbort.abort(); } catch (e) { /* already settled */ }
        }

        var controller = typeof AbortController === 'function' ? new AbortController() : null;
        var seq = (region.__awtSeq = (region.__awtSeq || 0) + 1);

        region.__awtAbort = controller;
        region.__awtPendingUrl = url;
        ...
        region.setAttribute('data-awt-table-busy', '');
        ...
        var settle = function () {
            if (seq !== region.__awtSeq) return;
            region.__awtPendingUrl = null;
            region.__awtAbort = null;
            region.removeAttribute('data-awt-table-busy');
        ...
        window.fetch(url, {
            ...
            headers: {
                'X-AWT-Table': region.dataset.awtTableId || '',
            ...
            Array.prototype.slice.call(doc.querySelectorAll('[data-awt-table]')).some(function (el) {
                if (el.dataset.awtTableId === id) { fresh = el; return true; }

                return false;
            });

            var freshViewport = fresh ? fresh.querySelector('[data-awt-table-viewport]') : null;
            if (!freshViewport) throw new Error('table fragment missing');

            viewport.innerHTML = freshViewport.innerHTML;
            region.dataset.awtTableState = fresh.dataset.awtTableState || region.dataset.awtTableState;
            ...
            region.dispatchEvent(new CustomEvent('awt:table:load', {
                bubbles: true,
    ...
    function tableLinkFor(target) {
        var retry = target.closest('[data-awt-table-retry]');
        if (retry) return { el: retry, url: retry.dataset.awtTableUrl };

        var link = target.closest('a[href]');
        if (!link) return null;
        if (!link.closest('.pagination') && !link.matches('[data-awt-table-link]')) return null;
    ...
    function submitForm(form) {
        var region = regionOf(form, form.dataset.awtTableForm);
        if (!region) return;

        var action = form.getAttribute('action') || region.dataset.awtTableEndpoint || window.location.pathname;
    ...
    document.addEventListener('submit', function (e) {
        var form = e.target.closest ? e.target.closest('form[data-awt-table-form]') : null;
        if (!form || !regionOf(form, form.dataset.awtTableForm)) return;
    ...
    document.addEventListener('change', function (e) {
        var form = e.target.closest ? e.target.closest('form[data-awt-table-form]') : null;
        if (!form || !regionOf(form, form.dataset.awtTableForm)) return;
        if (!form.hasAttribute('data-awt-table-auto')) return;
     ...
     */
    function restoreAll() {
        Array.prototype.slice.call(document.querySelectorAll('[data-awt-table]')).forEach(function (region) {
            if (region.__awtRestored) return;
            region.__awtRestored = true;
    ...
    function dtKey(settings) {
        ...
        var meta = document.querySelector('meta[name="awt-table-scope"]');
    ...
    function patchDataTables() {
        ...
        if (!DT || !DT.defaults || DT.defaults.__awtStatePatched) return;

        DT.defaults.__awtStatePatched = true;
    ...
    // after a visit are new elements with no __awtRestored marker of their own.
