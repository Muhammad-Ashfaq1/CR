/**
 * Service worker for Construction Ready PWA.
 *
...
self.addEventListener('fetch', (event) => {
  ...
  // Page navigations: network-first, one retry, then the offline page. Never cache HTML.
  if (request.mode === 'navigate') {
    event.respondWith(
      fetch(request)
        .catch(() => new Promise((resolve) => setTimeout(resolve, 1500)).then(() => fetch(request)))
