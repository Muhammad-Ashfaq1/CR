    ...
    <div class="awt-glass-intro">
        <div class="awt-glass-intro-copy">
            <h4 class="awt-glass-intro-title mb-1">
                <i class="icon-base ti tabler-calendar-check me-2 text-warning"></i> Daily Attendance Sheet
            </h4>
            <p class="awt-glass-intro-subtitle mb-0">Record daily attendance for site workers. Wages, shift multipliers, and wage history are computed automatically.</p>
        </div>
        <div class="awt-glass-intro-actions">
            <a href="{{ route('attendance.history') }}" class="btn btn-outline-secondary">
                <i class="icon-base ti tabler-history me-1"></i> Attendance History &rarr;
            </a>
        </div>
    </div>
</div>
        ...
        <form method="GET" action="{{ route('attendance.index') }}" class="row g-3 align-items-end" id="attendanceFilterForm">
            ...
            <div class="col-md-4">
                ...
                <div class="input-group">
                    ...
                    <span class="input-group-text bg-transparent text-muted">{{ $carbonDate->format('l') }}</span>
                </div>
            </div>
...
@if(!$selectedProjectId || $workers->isEmpty())
    <div class="awt-table-card awt-tone-secondary mb-4">
        <div class="awt-empty-state">
            ...
            <h5 class="awt-empty-state-title">No active workers found for this project</h5>
            <p class="awt-empty-state-desc">Make sure workers are registered and assigned to this project site.</p>
            <a href="{{ route('workers.create') }}" class="btn btn-primary btn-sm">
                <i class="icon-base ti tabler-plus me-1"></i> Register Worker
            </a>
        </div>
    </div>
@else
    <form action="{{ route('attendance.store-daily') }}" method="POST">
        ...
        <div class="awt-table-card awt-tone-secondary mb-4">
            <div class="awt-listing-toolbar">
                <div>
                    <h5 class="mb-0 fw-semibold">Site Roster ({{ $workers->count() }} Workers)</h5>
