{{--
    ...
    data-awt-theme + data-bs-theme), so there is no flash to fix in the normal
    case, and this script deliberately does NOT override that. Its only jobs:
      ...
      1. `system` mode — the server cannot read prefers-color-scheme, so it
         ...
         it. Resolving that here rather than in awt-theme.js moves it ahead of
...
(function () {
    ...
    var variant = html.getAttribute('data-awt-theme');
    var mode = html.getAttribute('data-awt-theme-mode');

    // (2) Server said nothing — fall back to the cache, if it is sane.
    if (VARIANTS.indexOf(variant) === -1) {
        try {
            var cached = JSON.parse(localStorage.getItem('awt_theme') || 'null');
    ...
    for (var i = 0; i < VARIANTS.length; i++) {
        html.classList.remove('awt-theme-' + VARIANTS[i]);
    }
    // High contrast is an overlay ON the dark theme — it wears both classes,
    // mirroring AppTheme::classesFor().
    if (rendered === 'high-contrast') html.classList.add('awt-theme-dark');
    html.classList.add('awt-theme-' + rendered);

    html.setAttribute('data-bs-theme', prefersDark ? 'dark' : 'light');
    html.setAttribute('data-awt-theme', rendered);
})();
</script>
